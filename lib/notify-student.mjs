 ...
 *
 * The sender here is "Admin", matching the label above a reply in the chat.
 * The EMAIL keeps the formal "Student Affairs Unit" (see adminMessageEmail) —
 * an official-looking sender is right for an inbox, a short one for a bubble.
 *
 * There are TWO bodies because the same sentence does not fit both places it
 * appears. The bell is read by a student who is already signed in and looking
 * at the app, so telling them to sign in is nonsense — and reads as though the
 * app does not know they are there. A push lands on a lock screen with no
 * context at all, so it has to say where to go.
 *
 * The message itself is never included: see the note on the email below.
 */
export const ADMIN_MESSAGE_TITLE = "New message from Admin";

/** In-app bell — the student is already inside, so no call to action. */
export const ADMIN_MESSAGE_BODY = "Admin has sent you a private message.";

/** Web Push — may arrive on a locked phone, so it points somewhere. */
export const ADMIN_MESSAGE_PUSH_BODY =
  "Admin has sent you a private message. Open the app to read it.";
...
/**
 * Absolute origin of the app, for the link in the email. Mirrors appBaseUrl()
 * in lib/email.ts (which this module cannot import — it is TypeScript) and
 * carries the same production guard: a deploy that sets neither APP_URL nor
 * NEXTAUTH_URL would otherwise mail every student a localhost link that looks
 * like a working email until someone clicks it.
 */
...
function appBaseUrl() {
  ...
  if (process.env.NODE_ENV === "production" && isLoopback(base) && !baseUrlWarned) {
    baseUrlWarned = true;
    console.error(
      "[student-connect:notify] APP_URL and NEXTAUTH_URL are both missing (or " +
        "both point at localhost) in production — the link in every message " +
        "email sent from this process is unreachable. Set APP_URL to the " +
        "public origin.",
    );
  }
...
/** True for a URL that only resolves on the machine that built it. */
...
export async function notifyNewAdminMessage({
  ...
  try {
    ...
    if (typeof push === "function") {
      try {
        await push(userId, ADMIN_MESSAGE_TITLE, ADMIN_MESSAGE_PUSH_BODY);
      } catch (err) {
    ...
    if (typeof sendEmail === "function" && typeof email === "string" && email.includes("@")) {
      try {
        ...
        // A transport that REPORTS failure rather than throwing — which is
        // exactly what lib/email.ts does by contract, and what a missing
        // RESEND_API_KEY produces — must not vanish. Without this the email
        // is skipped in total silence and the admin believes the student was
        // told. This is the same "the API said link sent while nothing was
        // delivered" failure that module's own comments document.
        if (result && result.ok === false) {
          const reason = result.error ?? "send failed";
          errors.push(`email: ${reason}`);
          console.error("[student-connect:notify] email not sent:", reason);
        }
      } catch (err) {
